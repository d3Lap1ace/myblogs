...
|Flag|Description|
|---|---|
|OFF|`"Initialize a new CLAUDE.md file with codebase documentation"`|
|ON|`"Initialize new CLAUDE.md file(s) and optional skills/hooks with codebase documentation"`|

---

## Diff Summary

|Dimension|OFF (old `J6Y`)|ON (new `M6Y`)|
|---|---|---|
|User interaction|None — fire and forget|Heavy — multiple `AskUserQuestion` rounds with previews|
|Output artifacts|`CLAUDE.md` only|`CLAUDE.md` + `CLAUDE.local.md` + `.claude/skills/` + hooks in `settings.json`|
|Phases|1 (implicit)|8 (explicit)|
|Personalization|None|Role, familiarity, sandbox URLs, communication prefs|
|Skills|Not supported|Full skill creation with YAML frontmatter|
|Hooks|Not supported|PostToolUse/PreToolUse/Stop hooks with validation flow|
|Git worktree awareness|No|Yes — detects and handles nested vs sibling worktrees|
|Monorepo support|Minimal|`.claude/rules/` + subdirectory CLAUDE.md suggestions|
|Proposal flow|No|Preview panel with accept/drop options|
|Plugin recommendations|No|frontend-design, playwright, skill-creator plugins|
|Existing file handling|"suggest improvements"|Explicit "propose diffs, explain why, don't silently overwrite"|
|`@import` syntax|No|Yes — references to external docs to avoid bloating CLAUDE.md|
